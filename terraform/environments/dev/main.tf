...
resource "azurerm_key_vault" "main" {
  ...
  purge_protection_enabled   = false   # dev: allow purge for fast iteration

  network_acls {
    default_action = "Allow"   # dev: open; lock down in prod
...
module "waf" {
  ...
  # TLS cert must be pre-loaded into Key Vault (gitignored).
  # Pass via: terraform apply -var='tls_cert_keyvault_secret_id=https://...'
  # or a gitignored terraform.tfvars file.
  tls_cert_keyvault_secret_id = var.tls_cert_keyvault_secret_id

  aks_kubelet_identity_object_id = module.aks.kubelet_identity

  # Dev: Detection mode — logs WAF matches without blocking, safe for tuning
  waf_mode           = "Detection"
...
module "frontdoor" {
  ...
  # Dev: Standard SKU is cheaper; no Private Link needed
  sku_name = "Standard_AzureFrontDoor"

  # Dev: Detection mode — logs without blocking, safe for pipeline tuning
  waf_mode                 = "Detection"
  ...
  # Custom domains: populate via gitignored terraform.tfvars or CI variable
...
# private DNS FQDN. Connection string is stored in Key Vault and mounted
# into pods via the CSI Key Vault driver (never in Kubernetes Secrets).
module "mysql" {
  ...
  # Dev: burstable tier, minimal storage, no HA
...
module "acr" {
  ...
  acr_name                          = "acrdev${replace(local.name_prefix, "-", "")}"
  resource_group_name               = azurerm_resource_group.main.name
  location                          = local.location
  sku                               = "Standard"
  public_network_access_enabled     = true  # Dev: public for ease of testing
...
module "velero" {
  ...
  resource_group_name  = azurerm_resource_group.main.name
  location             = local.location
  storage_account_name = "veldev${substr(replace(local.name_prefix, "-", ""), 0, 14)}"
  replication_type     = "LRS"
  backup_retention_days = 30
  aks_oidc_issuer_url  = module.aks.oidc_issuer_url
  # Allow the AKS subnet to access backup storage directly
  allowed_subnet_ids   = [module.vnet.aks_subnet_id]
  tags                 = local.tags
}

# ── DNS Zone ───────────────────────────────────────────────────────────────────────
# Public DNS zone for the dev domain.
# After `terraform apply`, copy the name_servers output to your registrar.
module "dns" {
  source = "../../modules/dns"
  ...
  # Subdomains (e.g. www, api, app) → Front Door CNAME
  subdomain_cname_records = {
    for subdomain in var.subdomains :
    subdomain => { target = module.frontdoor.endpoint_hostname }
  }

  # Front Door validation tokens — proves domain ownership for managed TLS certs
...
output "acr_login_server" {
  description = "ACR login server FQDN — use in Helm values and image references"
